...
def vectors_insert(vectors, datasetType='训练数据集'):
    if datasetType == '训练数据集':
        for vector in vectors:
            ...
            if '_id' in vector:
                vector.pop('_id')
            vectorSave = OriginalVector.from_json(json.dumps(vector)) # 原向量 -> json -> BaseDocument(mongoDB)中的
            vectorSave.save()
        ...
        for vector in vectors:
            ...
            if '_id' in vector:
                vector.pop('_id')
            if 'preprocessid' in vector:
                vector.pop('preprocessid')
            vectorSave = FeaturesBatchVector.from_json(json.dumps(vector))
            vectorSave.save()
    elif datasetType == '结果数据集':
        for vector in vectors:
            if '_cls' in vector:
                vector.pop('_cls')
            if '_id' in vector:
                vector.pop('_id')
            vectorSave = ResultBatchVector.from_json(json.dumps(vector))
            vectorSave.save()
    return
...
def vectors_select_divide(datasetid, deleted, limit, page, datasetType='训练数据集'):
    ...
    if datasetType == '训练数据集':
        ...
        vectors = ResultBatchVector.objects(Q(datasetid=datasetid))
